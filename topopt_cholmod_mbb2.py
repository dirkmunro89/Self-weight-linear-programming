...
def main(nelx,nely,volfrac,penal,rmin,ft):
	...
	while change>0.01 and loop<2000:
		...
		sK=((KE.flatten()[np.newaxis]).T*(Emin+(alpha*xPhys+(1-alpha)*xPhys**penal)*(Emax-Emin))).flatten(order='F')
		...
		obj=( (Emin+(alpha*xPhys+(1-alpha)*xPhys**penal)*(Emax-Emin))*ce ).sum()
		dc[:]=-1e0*(alpha + penal*(1-alpha)*xPhys**(penal-1)*(Emax-Emin))*ce
...
#	g=1e0-np.sum(x)/float(nelx*nely)/volfrac
		g=np.sum(x)/float(nelx*nely)/volfrac-1e0
